...
class ExploreNewsDetailScreen extends React.Component {
  state = {
    isLoading: true,
    moreNews: null,
  };
  ...
  onNewsItemSelectedHandler = item => {
    this.props.getNewsDetail({
      onSuccess: () => {
        this.props.navigation.push(routes.ExploreNewsDetailScreen);
        this.getMoreNewsFromDomainsUrl();
      },
      news: item,
      source: 'Explore',
...
const mapStateToProps = state => ({
  news: state.news.selectedExploreNews,
});
